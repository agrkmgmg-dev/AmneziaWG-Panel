...
class TrafficSyncService:
    ...
    async def sync(self) -> int:
        ...
        # Counters live on different interfaces.  Keeping their maps separate
        # prevents matching the same private IP across two VPN protocols.
        for protocol, collector in (
            ("amneziawg", AWGCollector()),
            ("wireguard", WGCollector()),
        ):
            records = collector.collect()
            peer_map = {
                peer.address: peer
                for peer in peers
                if getattr(peer, "protocol", "amneziawg") == protocol
            }
            ...
            for item in records:
                ...
                # Both `awg` and `wg` dump output uses cumulative counters.
