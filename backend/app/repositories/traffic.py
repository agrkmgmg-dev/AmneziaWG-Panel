...
class TrafficRepository(BaseRepository[Traffic]):
    ...
    def __init__(self, session: AsyncSession) -> None:
        super().__init__(session=session, model=Traffic)
    ...
    async def get_by_peer(
        ...
        result = await self.session.execute(
            ...
            .order_by(Traffic.created_at.desc())
        )
    ...
    async def get_total_upload(
        ...
        result = await self.session.execute(
            select(func.coalesce(func.sum(Traffic.upload_bytes), 0))
            .where(Traffic.peer_id == peer_id)
        )
    ...
    async def get_total_download(
        ...
        result = await self.session.execute(
            select(func.coalesce(func.sum(Traffic.download_bytes), 0))
            .where(Traffic.peer_id == peer_id)
        )
    ...
    async def get_total_traffic(
        ...
        result = await self.session.execute(
            select(
                func.coalesce(
                    func.sum(
                        Traffic.upload_bytes + Traffic.download_bytes
                    ),
                    0,
                )
            ).where(Traffic.peer_id == peer_id)
